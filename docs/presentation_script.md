...
L'enchaînement des simulations suit un fil conducteur volontaire :

```
Expérience réelle → 2D MCU → 3D Cône → 3D Membrane → Machine Learning
```
...
- Plein écran, aucun menu visible.
- Les touches **1 à 4** changent de simulation et lancent automatiquement.
...
> **Suggestion :** commencer sur le MCU (touche 1) pour débuter la démonstration.

---

## Conditions initiales scriptées conseillées

Pour que la comparaison cône / membrane soit parlante, utiliser les **mêmes conditions initiales** sur les deux (preset "Conditions de présentation") :

```
r0 = 0.55 m       # position radiale initiale
v0 = 0.55 m/s     # vitesse initiale
phi0 = 90°        # lancement tangentiel
friction = 0.012
```
...
> Ces valeurs correspondent au preset `F1 : Conditions de présentation` sur les deux simulations — appuyer sur F1 suffit à réinitialiser les deux.
