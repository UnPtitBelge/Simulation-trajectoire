...
from components.plot_3d.sim_membrane import build_animated_figure_3d
...
def register_all(app: dash.Dash):
    ...
    # 3) Update animated 3D simulation figure when the confirm button is clicked.
    @app.callback(
        [
    ...
    # 4) Reset inputs to defaults and refresh animated figure.
    @app.callback(
        [
    ...
    def switch_control_panel(active_tab):
        ...
        if active_tab == "static-3d-tab":
            # Static view removed; reuse dynamic controls to keep UI functional
            return control_panel_3D()
        if active_tab == "newton-2d-tab":
    ...
    def update_newton_2d(
        n_clicks, v0, theta_deg, radius, friction_coef, show_full_trajectory
    ):
        n_frames = 10000
