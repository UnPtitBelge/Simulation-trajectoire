"""Régression linéaire directe CI → trajectoire (version corrigée).
...
Limites connues de ce paradigme (non corrigées ici, voir ml/models.py) :
  - La taille de sortie est fixe : on ne peut pas prédire au-delà de target_len pas.
  - La régression linéaire ne peut pas capturer la physique non-linéaire (centrifuge,
    Coriolis) → grandes erreurs sur les trajectoires orbitales.

Usage :
    python src/scripts/demo_regression_lineaire.py
    python src/scripts/demo_regression_lineaire.py --test-id 5
    python src/scripts/demo_regression_lineaire.py --no-plot
"""
...
def plot_result(
    ...
    fig.suptitle(
        f"Régression linéaire directe — expID {test_id} (test)\n"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Régression linéaire directe CI→trajectoire (version corrigée)."
    )
    parser.add_argument("--test-id", type=int, default=None,
                        help="expID réservé au test (défaut : dernier)")
    parser.add_argument("--output",  type=Path,
                        default=ROOT.parent / "figures" / "demo_lr_direct.png",
                        help="Chemin de la figure (défaut : figures/demo_lr_direct.png)")
    parser.add_argument("--csv",     type=Path,
                        default=ROOT.parent / "results" / "demo_lr_direct.csv",
                        help="Chemin du CSV de métriques (défaut : results/demo_lr_direct.csv)")
    ...
    # ── Entraînement ──────────────────────────────────────────────────────────
    # Ridge (régularisation L2 légère) plutôt que LinearRegression pure :
    # évite les instabilités numériques quand la sortie est de haute dimension
    # (2 × target_len ≈ 600–800 valeurs).
    print("\nEntraînement de la régression Ridge (α=1.0)...")
    ...
    if not args.no_plot:
        plot_result(traj_vraie, traj_pred, test_id, len(X_train),
                    mae_r, target_len, args.output)
    else:
        plt.close("all")
