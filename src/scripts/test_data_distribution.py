...
def _hist2d(ax, x, y, bins, x_lim, y_lim, title, xlabel, ylabel, cmap="Blues"):
    h, xedges, yedges = np.histogram2d(
        x, y, bins=bins, range=[x_lim, y_lim], density=True
    )
    ax.imshow(
        h.T,
        origin="lower",
...
def plot_comparison(
    ...
    r_rand = rand_states[:, 0]
    r_grid = grid_states[:, 0]
    vr_rand = rand_states[:, 2]
    vr_grid = grid_states[:, 2]
    vt_rand = rand_states[:, 3]
    vt_grid = grid_states[:, 3]
    ...
    ax_r.hist(
        ...
        alpha=ALPHA,
        color="steelblue",
        label=f"Aléatoire ({len(rand_lengths):,} traj.)",
    )
    ax_r.hist(
        r_grid,
        bins=r_bins,
        ...
        alpha=ALPHA,
        color="darkorange",
        label=f"Grille ({len(grid_lengths):,} traj.)",
    )
    ...
    vmax_plot = float(
        np.percentile(np.abs(np.concatenate([vr_rand, vr_grid, vt_rand, vt_grid])), 99)
    )
...
def plot_lengths(rand_lengths: list[int], grid_lengths: list[int]) -> None:
    ...
    ax.hist(
        ...
        alpha=0.65,
        color="steelblue",
        label=f"Aléatoire — médiane {np.median(rand_lengths):.0f} pas",
    )
    ax.hist(
        grid_lengths,
        bins=bins,
        ...
        alpha=0.65,
        color="darkorange",
        label=f"Grille    — médiane {np.median(grid_lengths):.0f} pas",
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n-r", type=int, default=15, help="Grille : rayons (défaut : 15)"
    )
    parser.add_argument(
        "--n-theta", type=int, default=18, help="Grille : angles (défaut : 18)"
    )
    parser.add_argument(
        "--n-v", type=int, default=4, help="Grille : vitesses (défaut : 4)"
    )
    parser.add_argument(
        "--n-dir", type=int, default=8, help="Grille : directions (défaut : 8)"
    )
    ...
    print(
        f"CI grille      : {args.n_r} × {args.n_theta} × {args.n_v} × {args.n_dir}"
        f" = {n_grid_ci:,}"
    )
    ...
    print(
        f"  {len(rand_lengths):,} traj. valides  "
        f"({sum(rand_lengths):,} états  |  "
    ...
    print(
        f"  {len(grid_lengths):,} traj. valides  "
        f"({sum(grid_lengths):,} états  |  "
