...
class Window():
    ...
    def colorPickerButtons(self, parent):
        ...
        ttk.Button(cpButtons, text="Choose Color", command=self.onColorPicker).grid(row=0, column=0, sticky="ew", pady=(0, 8))

        ttk.Button(cpButtons, text="✓", command=self.getBGRValues).grid(row=0, column=1, sticky="ew", pady=(0, 8))
    ...
    def _tabTrackBall(self):
        ...
        self.labelVideoTrack.configure(image= ImageTk.PhotoImage(Image.new("RGBA", self.videoSize, (0, 0, 0, 255)))) # random pixels
        ...
        ttk.Label(right, text="Options", font=("Arial", 14, "bold")).grid(row=row, column=0, sticky="w", pady=(0, 10))
        row += 1
        ttk.Button(right, text="Choose File", command=self.onChooseFile).grid(row=row, column=0, sticky="w", pady=(0, 8))
        row += 1
        ...
        row += 1
        ttk.Label(right, text="Vitesse (unité?):").grid(row=row, column=0, sticky="w", pady=(0, 8))
        self.inputInitialSpeed = ttk.Entry(right, width=10)
        self.inputInitialSpeed.grid(row=row, column=1, sticky="w", pady=(0, 8))
        row += 1
        ttk.Button(right, text="Track Ball", command=self.getBGRValues).grid(row=row, column=0, sticky="w", pady=(0, 8))
        row += 1

        ttk.Button(right, text="▶", command=self.onPlayVideoTracked).grid(row=row, column=0, sticky="w", pady=(0, 8))
        row += 1
    ...
    def onChooseFile(self):
        ...
        self.filenameVideoTrack = tk.filedialog.askopenfilename(title='Open a video file', initialdir=path, filetypes=filetypes)
        if self.filenameVideoTrack:
            print(f"Selected file: {self.filenameVideoTrack}")


    def onPlayVideoTracked(self):
        if not hasattr(self, 'filenameVideoTrack') or not os.path.isfile(self.filenameVideoTrack):
            print("No valid video file selected.")
            return
        if self.player.live:
            print("A video is already playing.")
            return
        self.player = tkvideo(self.filenameVideoTrack, self.labelVideoTrack, loop=0, size=self.videoSize)
        self.player.play()
